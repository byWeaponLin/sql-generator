...
public class FromBuilderTest extends BaseTest {
    ...
    public void test_from_table_success() {
        SQLParameter sqlParameter = DSL.select()
                ...
                .from(table("students"))
                .build();
        assertEquals("SELECT id, name, score FROM students", sqlParameter.getSql());
        ...
        sqlParameter = DSL.select().column("id", "name", "score")
                ...
                .build();
        assertEquals("SELECT id, name, score FROM students AS stu", sqlParameter.getSql());
        assertEquals2(Lists.newArrayList(), sqlParameter.getParameters());
    }
    ...
    public void test_transform_operand_from_table_success() {
        SQLParameter sqlParameter = DSL.select()
                ...
                .column(name("name")._like_(PlaceholderOperand.value("林")))
                .column(name("age").gt(value(15)))
                .column(name("name").like_(value("林")))
                .from(table("students"))
                .build();
        assertEquals("SELECT id, name AS nm, MAX(score) AS maxScore, score + 10 AS newScore, name LIKE ?, age > 15, name LIKE '林%' FROM students", sqlParameter.getSql());
